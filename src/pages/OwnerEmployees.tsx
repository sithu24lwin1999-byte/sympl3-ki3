...
export default function OwnerEmployees() {
  ...
  const changeEmployeePassword = async (employee: Employee) => {
    if (!window.confirm(`Send a secure password change link to ${employee.email}?`)) return;
    try {
      await resetPassword(employee.email);
      if (shopId && user) await createRecord(`shops/${shopId}/auditLogs`, { shopId, actorId: user.id, actorName: user.name, action: 'EMPLOYEE_PASSWORD_CHANGE_LINK_SENT', detail: `${employee.name} (${employee.id})`, createdAt: new Date().toISOString() });
      notify(`Password change link sent to ${employee.email}.`);
    } catch (issue) { setFormError(issue instanceof Error ? issue.message : 'Unable to send password change link.'); }
  };
