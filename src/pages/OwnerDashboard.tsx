...
const COLORS = ['#5f6f52', '#a8663f', '#d4a373', '#2f8f83', '#8a6f4d', '#b66b62'];
...
export default function OwnerDashboard() {
  ...
  return <DashboardLayout role="OWNER">
    <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4 mb-8"><div><p className="mb-2 text-xs font-black uppercase tracking-[0.22em] text-[#a8663f]">Owner command room</p><h1 className="text-3xl font-bold tracking-tight text-[#1d1a16] mb-2">{shop?.name || 'My Shop'} Dashboard</h1><p className="text-[#736756]">Live sales, profitability, stock and customer performance.</p></div><Button variant="outline" className="bg-white" onClick={exportDaily} disabled={exporting!=='idle'}>{exporting==='loading'?<Loader2 className="w-4 h-4 mr-2 animate-spin"/>:exporting==='done'?<CheckCircle2 className="w-4 h-4 mr-2 text-emerald-500"/>:<Download className="w-4 h-4 mr-2"/>}{exporting==='loading'?'Exporting…':exporting==='done'?'Exported':'Export Daily Report'}</Button></div>
    ...
    <div className="grid lg:grid-cols-2 gap-8 mb-8"><ChartCard title="Sales by Hour (Today)"><BarChart data={analytics.salesByHour}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="label" interval={2}/><YAxis tickFormatter={shortMoney}/><Tooltip formatter={moneyTooltip}/><Bar dataKey="sales" fill="#5f6f52" radius={[5,5,0,0]}/></BarChart></ChartCard><ChartCard title="Daily Sales (14 Days)"><AreaChart data={analytics.daily}><defs><linearGradient id="dailySales" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#a8663f" stopOpacity={.3}/><stop offset="95%" stopColor="#a8663f" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="label"/><YAxis tickFormatter={shortMoney}/><Tooltip formatter={moneyTooltip}/><Area dataKey="sales" stroke="#a8663f" strokeWidth={3} fill="url(#dailySales)"/></AreaChart></ChartCard><ChartCard title="Weekly Sales (8 Weeks)"><BarChart data={analytics.weekly}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="label"/><YAxis tickFormatter={shortMoney}/><Tooltip formatter={moneyTooltip}/><Bar dataKey="sales" fill="#d4a373" radius={[5,5,0,0]}/></BarChart></ChartCard><ChartCard title="Monthly Sales (12 Months)"><AreaChart data={analytics.monthly}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="label"/><YAxis tickFormatter={shortMoney}/><Tooltip formatter={moneyTooltip}/><Area dataKey="sales" stroke="#5f6f52" strokeWidth={3} fill="#ebe4d3"/></AreaChart></ChartCard></div>
    <div className="grid lg:grid-cols-3 gap-8 mb-8"><PieCard title="Online vs In-store (30 Days)" data={analytics.channel}/><PieCard title="Payment Methods (30 Days)" data={analytics.payments}/><ChartCard title="Top-selling Products (30 Days)" compact><BarChart data={analytics.topProducts} layout="vertical"><CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e7d8c5"/><XAxis type="number"/><YAxis dataKey="name" type="category" width={95}/><Tooltip/><Bar dataKey="quantity" fill="#d4a373" radius={[0,5,5,0]}/></BarChart></ChartCard></div>
    <div className="grid lg:grid-cols-2 gap-8 mb-8"><ChartCard title="Top Categories (30 Days)"><BarChart data={analytics.topCategories}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="name"/><YAxis tickFormatter={shortMoney}/><Tooltip formatter={moneyTooltip}/><Bar dataKey="sales" fill="#a8663f" radius={[5,5,0,0]}/></BarChart></ChartCard><Card><div className="flex justify-between items-center mb-5"><h3 className="font-bold">Real-time Recent Orders</h3><Button variant="outline" className="h-9 text-xs" onClick={()=>navigate('/owner/orders')}>View All</Button></div><DataState empty={!loading&&orders.length===0} emptyMessage="No orders yet."/><div className="space-y-3">{orders.slice(0,8).map(order=><div key={order.id} className="flex items-center justify-between gap-3 rounded-2xl border border-[#eadbc8] bg-[#fffaf2] p-3"><div className="min-w-0"><p className="text-sm font-bold truncate">#{order.id.slice(-6)} · {order.customer||'Walk-in'}</p><p className="text-xs text-[#736756] truncate">{new Date(order.createdAt).toLocaleString()} · {order.type} · {order.paymentKind||order.paymentMethod}</p></div><div className="text-right shrink-0"><p className="text-sm font-bold">{formatCurrency(order.total)}</p><Badge variant={order.status==='COMPLETED'?'success':order.status==='CANCELLED'||order.status==='REFUNDED'?'danger':'warning'}>{order.status}</Badge></div></div>)}</div></Card></div>
  </DashboardLayout>;
}

function Metric({label,value,note,icon:Icon,accent}:{label:string;value:string;note:string;icon:React.ElementType;accent?:boolean}){return <Card className={cn('group h-full p-5 transition hover:-translate-y-0.5',accent&&'border-[#caa27d] bg-[#f4e5d5]')}><div className="flex justify-between"><p className={cn('text-xs font-black uppercase tracking-[0.16em]',accent?'text-[#865036]':'text-[#8a7b68]')}>{label}</p><span className={cn('grid h-9 w-9 place-items-center rounded-2xl',accent?'bg-[#fffdf8] text-[#865036]':'bg-[#f2dfcf] text-[#a8663f]')}><Icon className="h-4 w-4"/></span></div><p className="mt-4 text-2xl font-black text-[#1d1a16]">{value}</p><p className={cn('mt-2 text-xs',accent?'text-[#865036]':'text-[#736756]')}>{note}</p></Card>}
function ChartCard({title,children,compact=false}:{title:string;children:React.ReactElement;compact?:boolean}){return <Card><h3 className="font-bold mb-6 text-[#1d1a16]">{title}</h3><div className={compact?'h-72':'h-72'}><ResponsiveContainer width="100%" height="100%">{children}</ResponsiveContainer></div></Card>}
function PieCard({title,data}:{title:string;data:Array<{name:string;value:number}>}){return <Card><h3 className="font-bold mb-3 text-[#1d1a16]">{title}</h3>{data.length===0||data.every(item=>item.value===0)?<div className="h-72 grid place-items-center text-sm text-[#736756]">No sales data yet.</div>:<div className="h-72"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data} dataKey="value" nameKey="name" innerRadius={50} outerRadius={85} paddingAngle={3}>{data.map((item,index)=><Cell key={item.name} fill={COLORS[index%COLORS.length]}/>)}</Pie><Tooltip formatter={moneyTooltip}/><Legend/></PieChart></ResponsiveContainer></div>}</Card>}
