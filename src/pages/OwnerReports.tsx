...
const emptyTotals: ReportTotals = { sales: 0, orders: 0, initialPaid: 0, due: 0, purchases: 0, expenses: 0, dueCollected: 0 };
...
export default function OwnerReports() {
  ...
  const reportRows: Array<Array<string | number>> = [
    ['KI3 POS Accounting Report', `${startDate} to ${endDate}`], ['Metric', 'Amount'], ['Sales', totals.sales], ['Income received', income], ['Purchases', totals.purchases], ['Expenses', totals.expenses], ['Cost of goods', cogs], ['Gross profit', grossProfit], ['Net profit', netProfit], ['Outstanding credit', totals.due], ['Due collected', totals.dueCollected], [],
  ...
  const exportPdf = () => { const pdf = new jsPDF({ orientation: 'landscape' }); pdf.setFontSize(18); pdf.text('KI3 POS Accounting Report', 14, 16); pdf.setFontSize(10); pdf.text(`${startDate} to ${endDate}`, 14, 23); autoTable(pdf, { startY: 29, head: [['Metric', 'Amount (MMK)']], body: [['Sales', totals.sales], ['Income received', income], ['Purchases', totals.purchases], ['Expenses', totals.expenses], ['Cost of goods', cogs], ['Net profit', netProfit], ['Outstanding credit', totals.due]] }); autoTable(pdf, { head: [['Date', 'Order', 'Customer', 'Channel', 'Payment', 'Sales', 'Tax', 'Due']], body: details.orders.map(order => [new Date(order.createdAt).toLocaleDateString(), order.orderNumber || order.id, order.customer, order.type, order.paymentMethod, order.total, order.tax || 0, order.dueAmount || 0]) }); pdf.save(`${fileBase}.pdf`); };
  ...
  return <DashboardLayout role="OWNER"><div className="report-print space-y-7">
    ...
    {!loading&&!error&&<>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric title="Sales" value={totals.sales} note={`${totals.orders} completed orders`} icon={TrendingUp}/><Metric title="Income received" value={income} note={`Due collected ${formatCurrency(totals.dueCollected)}`} icon={Banknote}/><Metric title="Purchases & expenses" value={totals.purchases+totals.expenses} note={`Purchases ${formatCurrency(totals.purchases)}`} icon={TrendingDown}/><Metric title="Net profit" value={netProfit} note={`Gross profit ${formatCurrency(grossProfit)}`} icon={WalletCards} accent/></div>
      <div className="grid gap-6 xl:grid-cols-2"><ReportTable title="Profit & Loss" rows={[['Sales',totals.sales],['Cost of goods',-cogs],['Gross profit',grossProfit],['Operating expenses',-operatingExpenses],['Net profit',netProfit]]}/><ReportTable title="Cash Flow" rows={[['Sales cash received',Math.max(0,totals.sales-totals.due)],['Due collection received',totals.dueCollected],['Purchases paid',-totals.purchases],['Expenses paid',-totals.expenses],['Owner withdrawals',-ownerWithdrawals],['Net cash flow',income-totals.purchases-totals.expenses]]}/></div>
