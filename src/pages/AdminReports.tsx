...
export default function AdminReports() {
  ...
  return <DashboardLayout role="ADMIN">
    <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 mb-8"><div><p className="mb-2 text-xs font-black uppercase tracking-[0.22em] text-[#a8663f]">Platform finance</p><h1 className="text-3xl font-bold tracking-tight text-[#1d1a16] mb-2">Financial Reports</h1><p className="text-[#736756]">Subscriptions, renewals, payments and tenant system sales.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={()=>downloadCsv('ki3-subscriptions.csv',rows)}><Download className="w-4 h-4 mr-2"/>CSV</Button><Button variant="outline" onClick={()=>downloadExcel('ki3-subscriptions.xls',rows)}><Download className="w-4 h-4 mr-2"/>Excel</Button><Button variant="outline" onClick={exportPdf}><Download className="w-4 h-4 mr-2"/>PDF</Button></div></div>
    ...
    <div className="grid lg:grid-cols-2 gap-8 mb-8"><Card><h3 className="font-bold mb-6 text-[#1d1a16]">Subscription Revenue by Period</h3><div className="h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={monthly}><defs><linearGradient id="income" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#a8663f" stopOpacity={.32}/><stop offset="95%" stopColor="#a8663f" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="name"/><YAxis tickFormatter={x=>`${x/1000}k`}/><Tooltip formatter={(value:number)=>formatCurrency(value)}/><Area dataKey="revenue" stroke="#865036" strokeWidth={3} fill="url(#income)"/></AreaChart></ResponsiveContainer></div></Card><Card><h3 className="font-bold mb-6 text-[#1d1a16]">Recurring Revenue by Plan</h3><div className="h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={planData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7d8c5"/><XAxis dataKey="name"/><YAxis tickFormatter={x=>`${x/1000}k`}/><Tooltip formatter={(value:number)=>formatCurrency(value)}/><Bar dataKey="revenue" fill="#5f6f52" radius={[8,8,0,0]}/></BarChart></ResponsiveContainer></div></Card></div>
    <Card><h3 className="font-bold mb-6 text-[#1d1a16]">Payment History</h3><div className="overflow-x-auto"><table className="w-full text-left min-w-[900px]"><thead><tr className="border-b border-[#eadbc8] text-xs uppercase tracking-[0.14em] text-[#8a7b68]">{['Date','Shop','Plan','Period','Type','Status','Amount'].map(x=><th key={x} className="pb-3">{x}</th>)}</tr></thead><tbody className="divide-y divide-[#efe4d5]"><TableStateRow columns={7} loading={loading} error={error} empty={!loading&&transactions.length===0} emptyMessage="No subscription payments recorded."/>{transactions.map(x=><tr key={x.id} className="hover:bg-[#fffaf2]"><td className="py-4 text-xs">{(x.paidAt||x.createdAt).slice(0,10)}</td><td className="font-bold">{x.shopName}</td><td>{x.plan}</td><td className="text-sm">{x.periodStart} → {x.periodEnd}</td><td>{x.type}</td><td>{x.status}</td><td className="font-bold">{formatCurrency(x.amount)}</td></tr>)}</tbody></table></div></Card>
  </DashboardLayout>;
}
function ReportCard({title,value,icon:Icon}:{title:string;value:string;icon:React.ElementType}){return <Card className="group relative overflow-hidden p-5 transition hover:-translate-y-0.5 hover:shadow-xl"><div className="absolute right-4 top-4 rounded-2xl bg-[#f2dfcf] p-2 text-[#865036]"><Icon className="w-4 h-4"/></div><p className="text-xs font-black uppercase tracking-[0.16em] text-[#8a7b68]">{title}</p><p className="text-2xl font-black mt-4 text-[#1d1a16]">{value}</p></Card>}
