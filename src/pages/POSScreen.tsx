...
function Modal({ title, close, children, receipt = false }: { title: string; close(): void; children: ReactNode; receipt?: boolean }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4 backdrop-blur-sm"><motion.div role="dialog" aria-modal="true" aria-label={title} initial={{opacity:0,scale:.96}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:.96}} className={cn('w-full overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-slate-900', receipt ? 'receipt max-w-sm' : 'max-w-2xl')}><div className="flex items-center justify-between border-b bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950"><h2 className="flex items-center gap-2 text-lg font-black"><CheckCircle2 className="h-5 w-5 text-emerald-600"/>{title}</h2><button aria-label="Close dialog" onClick={close}><X/></button></div><div className="p-5">{children}</div></motion.div></div>; }
