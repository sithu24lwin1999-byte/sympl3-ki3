...
import { jewellerySummary } from '@/lib/jewellery';
...
export default function POSScreen() {
  ...
  const filteredProducts = sellableProducts.filter(product => (activeCategory === 'All' || product.category === activeCategory) && `${product.name} ${product.sku} ${product.barcode || ''} ${jewellerySummary(product.jewellery)}`.toLowerCase().includes(search.toLowerCase()));
  ...
  const cancelCurrent = async () => { if (cart.length && !await confirm({title:'Clear current order?',message:'All items, customer information and payment selections in this cart will be removed.',confirmLabel:'Clear Order',danger:true})) return; resetOrder(); };
  ...
  const holdOrder = async () => {
    ...
    try {
      ...
      const held = { orderNumber: `HOLD-${Date.now().toString().slice(-8)}`, shopId, branchId, branchName, employeeId: user.id, employeeName: user.name, type: orderType, customer: customer || 'Walk-in', customerPhone, discountPercent, deliveryCharge: Math.max(0, deliveryCharge), notes: notes.trim(), items: cart.map(item => ({ productId: item.product.id, name: item.product.name, quantity: item.quantity, price: salePrice(item.product), cost: item.product.cost })), heldAt };
  ...
  const handleCheckout = async () => {
    ...
    const order: Omit<Order, 'id'> = {
      ...
      items: cart.map(item => ({ productId: item.product.id, name: item.product.name, quantity: item.quantity, price: salePrice(item.product), cost: item.product.cost })),
  ...
  useEffect(() => {
    if (!showReceipt || !lastOrder || settings?.printer?.autoPrint !== true) return;
    const timer = window.setTimeout(() => window.print(), 350);
    return () => window.clearTimeout(timer);
  }, [lastOrder, settings?.printer?.autoPrint, showReceipt]);

  useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      const target = event.target as HTMLElement | null;
      const typing = target?.tagName === 'INPUT' || target?.tagName === 'TEXTAREA' || target?.tagName === 'SELECT';
      if (event.key === 'Escape') { if (showReceipt) setShowReceipt(false); else if (showHeld) setShowHeld(false); else if (!typing) void cancelCurrent(); return; }
      if (typing || busy) return;
      if (event.key === '/') { event.preventDefault(); searchRef.current?.focus(); }
      if (event.key === 'F2') { event.preventDefault(); changeOrderType(orderType === 'ONLINE' ? 'IN_STORE' : 'ONLINE'); }
      if (event.key === 'F4' && cart.length) { event.preventDefault(); void holdOrder(); }
      if (event.key === 'F8' && cart.length) { event.preventDefault(); void handleCheckout(); }
    };
    window.addEventListener('keydown', handler); return () => window.removeEventListener('keydown', handler);
  });

  const deleteHeldOrder = async (held: HeldOrder) => { if (await confirm({title:'Delete held order?',message:`${held.customer || 'Walk-in'}'s held cart will be permanently removed.`,confirmLabel:'Delete',danger:true})) await deleteRecord(`shops/${shopId}/heldOrders`, held.id); };
  ...
  return <div className="pos-shell h-screen overflow-hidden text-slate-900">
    <header className="pos-header flex h-16 items-center justify-between border-b bg-white px-3 shadow-sm md:h-20 md:px-6">
      <div className="flex items-center gap-3"><Button variant="ghost" className="px-2" onClick={() => user?.role === 'OWNER' ? navigate('/owner') : logout()}><ArrowLeft className="h-5 w-5 md:mr-2"/><span className="hidden md:inline">{t('Back')}</span></Button>{shop?.logo && <img src={shop.logo} alt={`${shop.name || 'Shop'} logo`} className="h-11 w-11 rounded-2xl border border-[#dfd0bc] bg-[#fffdf8] object-contain p-1 shadow-sm md:h-12 md:w-12" />}<div><h1 className="font-black md:text-xl">{shop?.name || 'KI3 POS'}</h1><p className="text-xs font-bold text-[#5f6f52]">{branchName} · {navigator.onLine ? t('Online') : t('Offline queue')}</p></div></div>
    ...
    <main className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden md:h-[calc(100vh-5rem)] lg:flex-row">
      <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div className="space-y-3 p-3 md:p-5">
          <div className="grid grid-cols-[auto_1fr_auto] gap-2"><div className="flex rounded-2xl bg-[#efe4d5] p-1"><button onClick={() => changeOrderType('IN_STORE')} className={cn('rounded-xl px-3 py-2 text-xs font-black text-[#6c5137]', orderType === 'IN_STORE' && 'bg-[#5f6f52] text-white shadow-sm')}>In-store</button><button onClick={() => changeOrderType('ONLINE')} className={cn('rounded-xl px-3 py-2 text-xs font-black text-[#6c5137]', orderType === 'ONLINE' && 'bg-[#5f6f52] text-white shadow-sm')}>Online</button></div><div className="relative"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#a39582]"/><Input ref={searchRef} value={search} onChange={event => setSearch(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { const exact = sellableProducts.find(product => product.barcode === search.trim()); if (exact) { addToCart(exact); setSearch(''); } } }} placeholder="Search name, SKU or barcode" className="h-12 rounded-2xl border-[#dfd0bc] bg-[#fffdf8] pl-11"/></div><button aria-label="Focus barcode scanner" onClick={() => searchRef.current?.focus()} className="grid h-12 w-12 place-items-center rounded-2xl bg-[#a8663f] text-white shadow-sm hover:bg-[#865036]"><ScanBarcode/></button></div>
          <div className="hidden items-center gap-3 text-[11px] font-semibold text-slate-400 xl:flex"><span><kbd>/</kbd> Search</span><span><kbd>F2</kbd> Order type</span><span><kbd>F4</kbd> Hold</span><span><kbd>F8</kbd> Pay</span><span><kbd>Esc</kbd> Close</span></div>
          <div className="flex gap-2 overflow-x-auto">{categories.map(category => <button key={category} onClick={() => setActiveCategory(category)} className={cn('whitespace-nowrap rounded-xl border px-4 py-2 text-sm font-bold', activeCategory === category ? 'border-[#5f6f52] bg-[#5f6f52] text-white' : 'border-[#dfd0bc] bg-[#fffdf8] text-[#736756] hover:border-[#cdbca7] hover:bg-[#f6ecdf]')}>{category}</button>)}</div>
        </div>
        <div className="flex-1 overflow-y-auto p-3 pt-0 md:p-5 md:pt-0"><DataState loading={productsLoading} error={productsError} empty={!productsLoading && !productsError && filteredProducts.length === 0} emptyMessage="No matching products available for this order type."/><div className="pos-product-grid grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">{filteredProducts.map(product => { const unavailable = stockTracked(product) && product.stock <= 0 && !allowNegativeStock; const jewelInfo = jewellerySummary(product.jewellery); return <motion.button whileTap={{scale:.97}} key={product.id} disabled={unavailable} onClick={() => addToCart(product)} className="pos-product-card overflow-hidden rounded-2xl border bg-white text-left shadow-sm disabled:opacity-50"><div className="relative h-28 bg-slate-100 md:h-36"><img src={product.image} alt={product.name} className="h-full w-full object-cover"/><Badge className="absolute right-2 top-2 bg-white text-slate-700">{product.category}</Badge></div><div className="p-3"><p className="truncate font-bold">{product.name}</p>{jewelInfo&&<p className="mt-1 truncate text-[11px] font-semibold text-amber-700">{jewelInfo}</p>}<div className="mt-1 flex items-end justify-between gap-2"><p className="font-black text-blue-600">{formatCurrency(salePrice(product))}</p><p className={cn('text-xs font-bold', product.stock <= 0 ? 'text-red-500' : 'text-slate-400')}>{stockTracked(product) ? `${product.stock} ${product.unit || 'pcs'}` : 'Service'}</p></div></div></motion.button>})}</div></div>
      </section>

      <aside className="pos-cart-panel flex max-h-[58vh] w-full flex-col border-l border-[#dfd0bc] bg-[#fffdf8] shadow-xl lg:max-h-none lg:w-[460px]">
        <div className="flex items-center justify-between border-b border-[#dfd0bc] bg-[#f0e5d8] p-3"><h2 className="flex items-center font-black"><ShoppingBag className="mr-2 h-5 w-5 text-[#5f6f52]"/>{orderType === 'ONLINE' ? 'Online' : 'In-store'} Order</h2><div className="flex gap-1"><Button variant="outline" className="h-9 border-[#ddcdbc] px-3 text-[#7a5137]" disabled={!cart.length || busy} onClick={holdOrder}><Pause className="mr-1 h-4 w-4"/>Hold</Button><Button variant="ghost" className="h-9 px-2 text-red-600" onClick={cancelCurrent}><Trash2 className="h-4 w-4"/></Button></div></div>
        <div className="min-h-24 flex-1 space-y-2 overflow-y-auto p-3">{cart.length === 0 ? <div className="grid h-full place-items-center text-center text-sm text-slate-400"><div><ShoppingBag className="mx-auto mb-2 h-12 w-12 opacity-20"/>Tap a product to start an order.</div></div> : cart.map(item => <div key={item.product.id} className="flex items-center gap-3 rounded-2xl border p-2"><img src={item.product.image} alt="" className="h-12 w-12 rounded-xl object-cover"/><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.product.name}</p><p className="text-sm font-black text-blue-600">{formatCurrency(salePrice(item.product) * item.quantity)}</p><p className="truncate text-[10px] text-slate-400">{jewellerySummary(item.product.jewellery) || `Stock ${item.product.stock}`}</p></div><div className="flex items-center gap-2 rounded-xl bg-slate-100 p-1"><button className="grid h-8 w-8 place-items-center rounded-lg bg-white" onClick={() => updateQuantity(item.product, -1)}><Minus className="h-4 w-4"/></button><b>{item.quantity}</b><button className="grid h-8 w-8 place-items-center rounded-lg bg-blue-100 text-blue-700" onClick={() => updateQuantity(item.product, 1)}><Plus className="h-4 w-4"/></button></div></div>)}</div>
        <div className="max-h-[54%] overflow-y-auto border-t border-[#dfd0bc] bg-[#f7f1e7] p-3">
          ...
          <Button disabled={!cart.length || busy || !activeShift} onClick={handleCheckout} className="mt-3 h-14 w-full rounded-2xl bg-emerald-600 text-base font-black text-white hover:bg-emerald-700"><Banknote className="mr-2 h-5 w-5"/>{busy ? 'Processing…' : !activeShift ? 'Open Shift to Sell' : `Complete Sale · ${formatCurrency(grandTotal)}`}</Button>
        </div>
      </aside>
    </main>

    <AnimatePresence>{showHeld && <Modal title={`Held Orders (${heldOrders.length})`} close={() => setShowHeld(false)}><div className="max-h-[65vh] space-y-3 overflow-auto">{heldOrders.length === 0 && <p className="py-10 text-center text-sm text-slate-400">No held orders.</p>}{heldOrders.map(held => <div key={held.id} className="rounded-2xl border p-4"><div className="flex justify-between gap-3"><div><p className="font-bold">{held.customer || 'Walk-in'} · {held.items.reduce((sum,item) => sum + item.quantity, 0)} items</p><p className="text-xs text-slate-400">{held.type === 'ONLINE' ? 'Online' : 'In-store'} · {new Date(held.heldAt).toLocaleString()} · {held.employeeName}</p></div><div className="flex gap-1"><Button className="h-9 px-3" onClick={() => resumeOrder(held)}><Play className="mr-1 h-4 w-4"/>Resume</Button><Button aria-label="Delete held order" variant="outline" className="h-9 px-2 text-red-600" onClick={() => void deleteHeldOrder(held)}><Trash2 className="h-4 w-4"/></Button></div></div></div>)}</div></Modal>}</AnimatePresence>
...
function PaymentButton({ label, active, onClick }: { label: string; active: boolean; onClick(): void }) { const tone = label === 'Cash' ? 'border-emerald-200 text-emerald-700' : label === 'Card' ? 'border-blue-200 text-blue-700' : label === 'Credit' ? 'border-orange-200 text-orange-700' : label === 'Split' ? 'border-purple-200 text-purple-700' : 'border-cyan-200 text-cyan-700'; return <button onClick={onClick} className={cn('min-h-14 rounded-xl border bg-white p-1 text-[11px] font-black transition-colors hover:bg-slate-50', tone, active && 'bg-blue-50 ring-2 ring-blue-100')}><CreditCard className="mx-auto mb-1 h-4 w-4"/>{label}</button>; }
...
function Modal({ title, close, children, receipt = false }: { title: string; close(): void; children: ReactNode; receipt?: boolean }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/50 p-4 backdrop-blur-sm"><motion.div role="dialog" aria-modal="true" aria-label={title} initial={{opacity:0,scale:.96}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:.96}} className={cn('w-full overflow-hidden rounded-3xl bg-white shadow-2xl', receipt ? 'receipt max-w-sm' : 'max-w-2xl')}><div className="flex items-center justify-between border-b border-blue-100 bg-blue-50 p-4"><h2 className="flex items-center gap-2 text-lg font-black"><CheckCircle2 className="h-5 w-5 text-emerald-600"/>{title}</h2><button aria-label="Close dialog" onClick={close}><X/></button></div><div className="p-5">{children}</div></motion.div></div>; }
function Receipt({ order, shop, settings }: { order: Order; shop?: Shop; settings?: ShopSettings }) { return <div style={{width:settings?.receipt?.paperWidth||'80mm',maxWidth:'100%'}}>{settings?.receipt?.header&&<p className="mb-2 text-center text-sm font-bold">{settings.receipt.header}</p>}<div className="text-center">{settings?.receipt?.showLogo!==false&&shop?.logo&&<img src={shop.logo} alt={`${shop.name || 'Shop'} logo`} className="mx-auto mb-2 h-14 max-w-32 object-contain" />}<h2 className="text-2xl font-black">{shop?.name || 'KI3 POS'}</h2><p className="text-sm text-slate-500">{shop?.address || shop?.phone}</p><p className="mt-1 text-xs text-slate-400">#{settings?.invoicePrefix || 'KI3'}-{order.id} · {new Date(order.createdAt).toLocaleString()}</p><p className="text-xs font-bold text-blue-600">{order.type === 'ONLINE' ? 'Online' : 'In-store'} order</p></div><div className="my-5 space-y-2 border-y border-dashed py-4">{order.items.map(item => <div key={item.productId} className="flex justify-between text-sm"><b>{item.quantity}× {item.name}</b><b>{formatCurrency(item.price * item.quantity)}</b></div>)}</div><div className="space-y-1 text-sm"><TotalRow label="Subtotal" value={order.subtotal}/>{settings?.receipt?.showTax!==false&&<TotalRow label="Tax" value={order.tax}/>}<TotalRow label="Discount" value={-order.discount}/>{Boolean(order.serviceCharge) && <TotalRow label="Service charge" value={order.serviceCharge || 0}/>} {Boolean(order.deliveryCharge) && <TotalRow label="Delivery" value={order.deliveryCharge || 0}/>}<div className="flex justify-between border-t pt-2 text-lg font-black"><span>Total</span><span>{formatCurrency(order.total)}</span></div></div><div className="mt-4 rounded-xl bg-slate-50 p-3 text-sm"><b>Payment</b>{(order.payments || []).map((payment,index) => <p key={`${payment.kind}-${index}`} className="flex justify-between"><span>{payment.label}</span><span>{formatCurrency(payment.amount)}</span></p>)}{order.notes && <p className="mt-2 border-t pt-2"><b>Notes:</b> {order.notes}</p>}</div>{settings?.receipt?.footer&&<p className="mt-4 text-center text-xs text-slate-500">{settings.receipt.footer}</p>}</div>; }
