    ...
    <main className="flex h-[calc(100vh-4rem)] flex-col overflow-hidden md:h-[calc(100vh-5rem)] lg:flex-row">
      <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div className="space-y-3 p-3 md:p-5">
          <div className="grid grid-cols-[auto_1fr_auto] gap-2"><div className="flex rounded-2xl bg-blue-100 p-1"><button onClick={() => changeOrderType('IN_STORE')} className={cn('rounded-xl px-3 py-2 text-xs font-black text-blue-700', orderType === 'IN_STORE' && 'bg-blue-600 text-white shadow-sm')}>In-store</button><button onClick={() => changeOrderType('ONLINE')} className={cn('rounded-xl px-3 py-2 text-xs font-black text-blue-700', orderType === 'ONLINE' && 'bg-blue-600 text-white shadow-sm')}>Online</button></div><div className="relative"><Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400"/><Input ref={searchRef} value={search} onChange={event => setSearch(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') { const exact = sellableProducts.find(product => product.barcode === search.trim()); if (exact) { addToCart(exact); setSearch(''); } } }} placeholder="Search name, SKU or barcode" className="h-12 rounded-2xl border-blue-100 bg-white pl-11"/></div><button aria-label="Focus barcode scanner" onClick={() => searchRef.current?.focus()} className="grid h-12 w-12 place-items-center rounded-2xl bg-purple-600 text-white shadow-sm hover:bg-purple-700"><ScanBarcode/></button></div>
      ...
      <aside className="pos-cart-panel flex max-h-[58vh] w-full flex-col border-l border-blue-100 bg-white shadow-xl lg:max-h-none lg:w-[460px]">
        <div className="flex items-center justify-between border-b border-blue-100 bg-blue-50 p-3"><h2 className="flex items-center font-black"><ShoppingBag className="mr-2 h-5 w-5 text-blue-600"/>{orderType === 'ONLINE' ? 'Online' : 'In-store'} Order</h2><div className="flex gap-1"><Button variant="outline" className="h-9 border-purple-200 px-3 text-purple-700" disabled={!cart.length || busy} onClick={holdOrder}><Pause className="mr-1 h-4 w-4"/>Hold</Button><Button variant="ghost" className="h-9 px-2 text-red-600" onClick={cancelCurrent}><Trash2 className="h-4 w-4"/></Button></div></div>
        ...
        <div className="max-h-[54%] overflow-y-auto border-t border-blue-100 bg-[#f8fbff] p-3">
