...
export default function AdminShops() {
  ...
  const remove = async (shop: Shop) => {
    setActiveMenu(null); setActionError('');
    const confirmation = window.prompt(`Permanently delete ${shop.name} and all tenant data?\n\nType the Shop ID to confirm:\n${shop.id}`);
    if (confirmation === null) return;
    if (confirmation.trim() !== shop.id) { setActionError('Shop ID did not match. Nothing was deleted.'); return; }
    setDeletingId(shop.id);
    try {
      await audit('SHOP_DELETE_REQUESTED', `${shop.name} (${shop.id})`);
      await updateDoc(doc(db, 'shops', shop.id), { status: 'SUSPENDED', systemStatus: 'STOPPED', updatedAt: new Date().toISOString() });
      await deleteShopCascade(shop.id);
      await audit('SHOP_DELETED', `${shop.name} (${shop.id})`);
      if (selected?.id === shop.id) setSelected(null);
      notify('Shop and tenant data permanently deleted. Its Firebase Auth identity remains unavailable on the Spark plan.');
    } catch (issue) {
      setActionError(issue instanceof Error ? issue.message : 'Unable to delete this shop.');
    } finally {
      setDeletingId('');
    }
  };
  ...
  return <DashboardLayout role="ADMIN">
    <div className="flex justify-between items-end mb-8"><div><p className="mb-2 text-xs font-black uppercase tracking-[0.22em] text-[#a8663f]">Tenant operations</p><h1 className="text-3xl font-bold tracking-tight text-[#1d1a16] mb-2">Tenant Shops</h1><p className="text-[#736756]">Subscriptions, access and tenant lifecycle management.</p></div><Button className="gap-2" onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> Create New Shop</Button></div>
...
function Modal({ title, close, children }: { title: string; close(): void; children: React.ReactNode }) { return <div className="fixed inset-0 z-50 bg-[#1d1a16]/45 backdrop-blur-sm flex items-center justify-center p-4"><div className="bg-[#fffdf8] rounded-3xl w-full max-w-xl max-h-[90vh] overflow-auto shadow-2xl border border-[#dfd0bc]"><div className="sticky top-0 p-5 bg-[#f3eadf] border-b border-[#eadbc8] flex justify-between z-10"><h2 className="text-xl font-bold text-[#1d1a16]">{title}</h2><button onClick={close} className="rounded-full p-2 text-[#736756] hover:bg-[#fffdf8]"><X className="w-5 h-5"/></button></div><div className="p-6">{children}</div></div></div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="block text-sm font-bold text-[#5f6f52] mb-2">{label}</span>{children}</label>; }
function ReadOnly({ label, value }: { label: string; value: string }) { return <div className="rounded-2xl bg-[#f3eadf] p-3 border border-[#eadbc8]"><p className="text-[10px] uppercase font-black tracking-[0.12em] text-[#8a7b68]">{label}</p><p className="text-sm font-semibold break-all text-[#1d1a16]">{value}</p></div>; }
